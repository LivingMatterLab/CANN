...
def get_hgo_weights():
    ...
    i1 = 0
    i2 = 10
    correlation_mat = np.zeros((n_terms, n_terms))
    correlation_mat[i1, i1] = sigma1
    correlation_mat[i2, i1] = rho * sigma2
    correlation_mat[i2, i2] = sigma2 * np.sqrt(1 - rho * rho)
    mean_weights = [0 for i in range(n_terms * 2)]
    mean_weights[0] = 1.0
    mean_weights[1] = 1.0 / 1.94149394456498 ** 0.5
    mean_weights[20] = 0.441
    mean_weights[21] = 1.0 / 3.60841024438425 ** 0.5
    model_weights = [np.array([mean_weights[i]]) for i in range(n_terms * 2)] + [correlation_mat]
...
def traindata(modelFit_mode, model_UT, lam_ut, P_ut, model_SS, gamma_ss, P_ss, model, midpoint, is_bcann=True, should_normalize=False):
    if modelFit_mode == 'T':
        ...
        sample_weights_45 = [np.concatenate([np.ones_like(np.array_split(y, 5)[test]) * ((1.0 / np.max(np.array_split(y, 5)[test]) if should_normalize else 1.0)
                                        if ((10 + 2 * test + axis) in modes or (19 - 2 * test - axis) in modes) else 0.0)
                             for y in np.array_split(output_train[1][axis], 5) for test in range(5)]) for axis in range(2)] ###### I think i put the wrong thing here
