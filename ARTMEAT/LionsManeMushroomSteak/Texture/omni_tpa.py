...
# SETUP DATA TO EXPORT, ADD NEW LISTS FOR EACH SAMPLE
export_data = {
    "Time" : [],
    "Mean IP Omni Force" : [],
    "STD IP Omni Force" : [],
...
# READ EXCEL OR TXT FILES, DON'T NEED TO CHNAGE
def read_data_file(file_path, sheet_name=None):
    if file_path.endswith(('.xlsx', '.xls')):
        ...
        return pd.read_csv(file_path, sep="\t", skiprows=0)

# DEFINE RADIUS AND SURFACE AREA 
radius = 4
...
# CHANGE THE VARIABLE NAMES AND FILE PATHS TO MATCH THE SAMPLES
# Additional in-plane files
meati_filenames_inplane = [f'/Users/lucasboyle/Documents/Living Matter Lab/Meati TPA Data/double-compression-slow-inplane-{i}.txt' for i in range(1, 11)]
meati_filenames_outplane = [f'/Users/lucasboyle/Documents/Living Matter Lab/Meati TPA Data/double-compression-slow-outplane-{i}.txt' for i in range(1, 11)]
omni_filenames_inplane = [f'/Users/lucasboyle/Documents/Living Matter Lab/Omni TPA Data/omni_TPA_inplane{i}.xlsx' for i in range(1, 11)]
omni_filenames_outplane = [f'/Users/lucasboyle/Documents/Living Matter Lab/Omni TPA Data/omni_TPA_cross{i}.xlsx' for i in range(1, 11)]

meati_data_outplane = process_force_curves('25/s', (132/255, 60/255, 12/255, 1.0), meati_filenames_outplane)  # Darkest
omni_data_outplane = process_force_curves('25/s', (132/255, 60/255, 12/255, 1.0), omni_filenames_outplane)  # Darkest
meati_data_inplane = process_force_curves('25/s', (132/255, 60/255, 12/255, 1.0), meati_filenames_inplane)  # Darkest
omni_data_inplane = process_force_curves('25/s', (132/255, 60/255, 12/255, 1.0), omni_filenames_inplane)  # Darkest

# CHANGE VARIABLE NAMES AND COLORS TO MATCH DATA
# Combined Plots for Each Speed with Blue and Red
for (data, label, color) in [
    (meati_data_outplane, "Meati Inplane", (132/255, 60/255, 12/255, 1.0)),
    (meati_data_inplane, "Meati Cross-Plane", (244/255, 177/255, 131/255, 1.0)),
    (omni_data_outplane, "Omni Inplane", "darkblue"),
    (omni_data_inplane, "Omni Cross-Plane", "dodgerblue")
]:
    # Plot inplane data in red
    time, mean_force, std_force, old_color, old_label = data
    plt.plot(time, mean_force, color=color, linestyle='-', linewidth=3, label=f'{label}')
    plt.fill_between(time, mean_force - std_force, mean_force + std_force, color=color, alpha=0.3)

    # SETUP EXPORT DATA
    if label == "Meati Inplane":
        export_data["Time"] = time
        export_data["Mean IP Meati Force"] = mean_force
        export_data["STD IP Meati Force"] = std_force
    elif label == "Meati Cross-Plane"   :
        export_data["Mean CP Meati Force"] = mean_force
        export_data["STD CP Meati Force"] = std_force
    elif label == "Omni Inplane":
        export_data["Mean IP Omni Force"] = mean_force
        export_data["STD IP Omni Force"] = std_force
    elif label == "Omni Cross-Plane":
        export_data["Mean CP Omni Force"] = mean_force
        export_data["STD CP Omni Force"] = std_force

plt.xlabel("Time [s]")
plt.ylabel("Force [N]")
plt.legend()
plt.title("Double Compression - 25%/s (n=10)")
plt.xlim(left=0)
plt.ylim(bottom=0) 
# plt.xticks([])
# plt.yticks([])
...
# PLOT CROSS PLANE DATA SEPARATELY
for (data, label, color) in [
    (meati_data_outplane, "Meati Cross-Plane", (132/255, 60/255, 12/255, 1.0)),
    (omni_data_outplane, "Omni Cross-Plane", "darkblue"),
]:
    time, mean_force, std_force, old_color, old_label = data
    time = np.linspace(0, 1, len(time))  # Ensuring uniform time axis
    plt.plot(time, mean_force, color=color, linewidth=3, label=f'{label}')
    plt.fill_between(time, mean_force - std_force, mean_force + std_force, color=color, alpha=0.3)
...
# plt.gca().spines['bottom'].set_visible(False)
plt.tight_layout()
# plt.title("Double Compression (Cross-Plane) - 25%/s (n=10)")
plt.show()


# PLOT INPLANE DATA SEPARATELY
for (data, label, color) in [
    (meati_data_inplane, "Meati Inplane", (244/255, 177/255, 131/255, 1.0)),
    (omni_data_inplane, "Omni Inplane", "dodgerblue"),
]:
    time, mean_force, std_force, old_color, old_label = data
    time = np.linspace(0, 1, len(time))  # Ensuring uniform time axis
    plt.plot(time, mean_force, color=color, linewidth=3, label=f'{label}')
...
# plt.gca().spines['bottom'].set_visible(False)
plt.tight_layout()
# plt.title("Double Compression (Inplane) - 25%/s (n=10)")
plt.show()
...
# CHANGE VARIABLE NAMES TO MATCH DATA
# Compute stats for outplane and inplane data
omni_outplane_stats, omni_outplane_stiffness, omni_outplane_hardness, omni_outplane_cohesiveness, omni_outplane_springiness, omni_outplane_resilience, omni_outplane_chewiness = compute_tpa_stats(omni_filenames_outplane)
omni_inplane_stats, omni_inplane_stiffness, omni_inplane_hardness, omni_inplane_cohesiveness, omni_inplane_springiness, omni_inplane_resilience, omni_inplane_chewiness = compute_tpa_stats(omni_filenames_inplane)
meati_outplane_stats, meati_outplane_stiffness, meati_outplane_hardness, meati_outplane_cohesiveness, meati_outplane_springiness, meati_outplane_resilience, meati_outplane_chewiness = compute_tpa_stats(meati_filenames_outplane)
meati_inplane_stats, meati_inplane_stiffness, meati_inplane_hardness, meati_inplane_cohesiveness, meati_inplane_springiness, meati_inplane_resilience, meati_inplane_chewiness = compute_tpa_stats(meati_filenames_inplane)
...
# CHANGE VAIABLE NAMES TO MATCH DATA
# # Perform Welch's t-tests for continuous variables and Mann-Whitney U test for bounded variables
print("Omni TPA Statistics:")
for param, (outplane_mean, outplane_std) in omni_outplane_stats.items():
    inplane_mean, inplane_std = omni_inplane_stats[param]
    
    if param in ["stiffness", "hardness", "chewiness"]:  # Welch’s t-test for continuous data
        t_stat, p_val = welch_t_test(eval(f'omni_outplane_{param}'), eval(f'omni_inplane_{param}'))
    else:  # Mann-Whitney U test for bounded data
        u_stat, p_val = mann_whitney_u_test(eval(f'omni_outplane_{param}'), eval(f'omni_inplane_{param}'))
...
print("Meati TPA Statistics:")
for param, (outplane_mean, outplane_std) in meati_outplane_stats.items():
    inplane_mean, inplane_std = meati_inplane_stats[param]
    
    if param in ["stiffness", "hardness", "chewiness"]:  # Welch’s t-test for continuous data
        t_stat, p_val = welch_t_test(eval(f'meati_outplane_{param}'), eval(f'meati_inplane_{param}'))
    else:  # Mann-Whitney U test for bounded data
        u_stat, p_val = mann_whitney_u_test(eval(f'meati_outplane_{param}'), eval(f'meati_inplane_{param}'))
...
# CHANGE VARIABLES TO MATCH DAYA
# Define your stats dictionaries
datasets = {
    "Meati Inplane": meati_inplane_stats,
    "Omni Inplane": omni_inplane_stats,
    "Meati Cross-Plane": meati_outplane_stats,
    "Omni Cross-Plane": omni_outplane_stats
}
...
# DON'T NEED TO CHANGE
# Create 6 bar plots
fig, axes = plt.subplots(2, 3, figsize=(18, 10))  # 2 rows x 3 columns grid
axes = axes.flatten()

# CHANGE COLORS FOR PLOTTING
for idx, metric in enumerate(metrics):
    ax = axes[idx]

    means = [datasets[name][metric][0] for name in datasets]
    stds = [datasets[name][metric][1] for name in datasets]

    # X positions for each dataset
    x = np.arange(len(datasets))

    # Create bar plot with error bars
    ax.bar(x, means, yerr=stds, capsize=8, alpha=0.7, color=[(132/255, 60/255, 12/255, 1.0), "dodgerblue", (244/255, 177/255, 131/255, 1.0), "darkblue"], edgecolor="black", linewidth=1.5)
    ax.set_xticks(x)
    ax.set_xticklabels(datasets.keys(), rotation=20)
    ax.set_ylabel(units[idx])
    ax.set_title(metric.capitalize())

plt.suptitle("Texture Profile Analysis (TPA) Characteristic Comparison")
plt.tight_layout()
plt.show()
...
#         val = val.tolist()
#     if len(val) < max_len:
#         val = val + [None] * (max_len - len(val))
#     export_data[key] = val

# df = pd.DataFrame(export_data)

# df.to_excel('/Users/lucasboyle/Desktop/Meati and Omni TPA Data.xlsx', index=False)
...
# def calculate_stiffness_compression(strain_curves, stress_curves):
#     stiffness_values = []
...
#     data = read_data_file(file_name, sheet_name=1)
...
# out_stiffness_values = calculate_stiffness_compression(out_strain_curves, out_stress_curves)
# in_stiffness_values = calculate_stiffness_compression(in_strain_curves, in_stress_curves)
...
# out_stress_curves = []
# for i in range(1, 11):
#     file_name = f'/Users/lucasboyle/Documents/Living Matter Lab/Omni TPA Data/omni_TPA_cross{i}.xlsx'
#     data = read_data_file(file_name, sheet_name=1)
#     data_num = data.iloc[2:].to_numpy(dtype=np.float64)
#     strain = data_num[:, 4] / data_num[0, 4]
#     stress = data_num[:, 3] / surf * 1000
...
# in_stress_curves = []
# for i in range(1, 11):
#     file_name = f'/Users/lucasboyle/Documents/Living Matter Lab/Omni TPA Data/omni_TPA_inplane{i}.xlsx'
#     data = read_data_file(file_name, sheet_name=1)
#     data_num = data.iloc[2:].to_numpy(dtype=np.float64)
#     strain = data_num[:, 4] / data_num[0, 4]
#     stress = data_num[:, 3] / surf * 1000
...
# #%% shear all
# # Function to calculate stiffness using linear regression with intercept fixed at (0,0)
# def calculate_stiffness_shear(strain_curves, stress_curves):
#     stiffness_values = []
...
#     data = read_data_file(file_name, sheet_name=2)
#     data_num = data.iloc[2:].to_numpy(dtype=np.float64)
...
# out_stiffness_values = calculate_stiffness_shear(out_strain_curves, out_stress_curves)
# in_stiffness_values = calculate_stiffness_shear(in_strain_curves, in_stress_curves)
...
#     data = read_data_file(filename, sheet_name=sheet_name)
#     data_num = data.iloc[2:].to_numpy(dtype=np.float64)
...
# outplane_stats = compute_survey_stats(outplane_data)
# inplane_stats = compute_survey_stats(inplane_data)
