...
def plotTen(ax, lam_ut_all,lam_ut_single, P_ut_all, Stress_predict_UT, Region, path2saveResults, modelFit_mode):
    ...
    ax.scatter(lam_ut_all, P_ut_all, s=10, zorder=31, lw=4, facecolors='w', edgecolors='k', clip_on=False)
    ax.plot(lam_ut_single, Stress_predict_UT, 'k', zorder=31, lw=6, color='darkorange')
    plt.tight_layout()
    plt.savefig(path2saveResults + '/TenPlot_' + 'Train' + modelFit_mode + '_' + 'Region' + Region + '.pdf')
    return


def plotCom(ax, lam_ut_all, lam_ut_single, P_ut_all, Stress_predict_UT, Region, path2saveResults, modelFit_mode):
    ...
    if Region == 'PB_SAUS':
        ...
        ax.set_yticklabels(['', '', '', ''])
        ax.set_ylim(0., -4.745)
    elif Region == 'outplane': 
        ax.set_yticks([0, 10, 20, 30, 40])
        #ax.set_yticklabels(['', '', '', ''])
        ax.set_ylim(0., 40)
        ax.set_xticks([1, 0.9, 0.8, 0.7, 0.6, 0.5])
        ax.tick_params(axis='both', labelsize=30) 
        #ax.set_xticklabels(['', '', '', '', '', ''])
        ax.set_xlim(1, 0.5)
        ax.scatter(lam_ut_all[::2], -P_ut_all[::2], s=50, zorder=31, lw=4, facecolors='k', edgecolors='k', clip_on=False)
        ax.plot(lam_ut_single, -Stress_predict_UT, 'k', zorder=31, lw=10, color='blue',linestyle='-')
        plt.tight_layout()
        ...
        ax.set_xlim(1, 0.5)
        ax.scatter(lam_ut_all[::2], -P_ut_all[::2], s=50, zorder=31, lw=4, facecolors='k', edgecolors='k', clip_on=False)
        ax.plot(lam_ut_single, -Stress_predict_UT, 'k', zorder=31, lw=6, color='red')
        plt.tight_layout()
        plt.savefig(path2saveResults + '/CompPlot_' + 'Train' + modelFit_mode + '_' + 'Region' + Region + '.pdf')
...
def plotMapCom(ax, Psi_model, model_weights, model_UT, terms, lam_ut_all, lam_ut_single, P_ut_all, Region, path2saveResults, modelFit_mode, model_type):
    # define number of terms for colormap plotting; purpose is to keep term colors the same for 1 or 2-term models
    if model_type == 'Invariant':
        numTerms = 8
    cmap = plt.cm.get_cmap('jet_r', numTerms)  # define the colormap
    ...
    if Region == 'PB_SAUS':
       ...
       #ax.set_xticks([1, 0.9, 0.8, 0.7, 0.6, 0.5])
        ...
        ax.set_xlim(1, 0.5)
        ax.set_xticks([])
        color_map_zero(ax, lam_ut_single, model_UT, model_weights, Psi_model, cmaplist, terms, model_type)
        ax.scatter(lam_ut_all[::2], -P_ut_all[::2], s=150, zorder=35, lw=1.5, facecolors='none', edgecolors='k', clip_on=False)
        ...
        ax.set_xlim(1, 0.5)
        color_map_zero(ax, lam_ut_single, model_UT, model_weights, Psi_model, cmaplist, terms, model_type)
        ax.scatter(lam_ut_all, -P_ut_all, s=10, zorder=35, lw=3, facecolors='red', edgecolors='k', clip_on=False)
        plt.tight_layout(pad=2)
        #plt.savefig(path2saveResults + '/CompCmap_' + 'Train' + modelFit_mode + '_' + 'Region' + Region + '.pdf')
